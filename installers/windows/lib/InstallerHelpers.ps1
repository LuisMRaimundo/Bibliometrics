...
function Initialize-ProjectVenv {
    ...
    $extras = $cfg.PipExtras
    Write-InstallLog "Installing $($cfg.AppName) and optional features (may take 10-25 minutes)..."
    & $venvPython -m pip install --upgrade pip wheel setuptools
    if ($LASTEXITCODE -ne 0) { throw 'pip upgrade failed.' }
    & $venvPython -m pip install -e "${ProjectRoot}[${extras}]"
    if ($LASTEXITCODE -ne 0) { throw "pip install -e .[${extras}] failed." }
